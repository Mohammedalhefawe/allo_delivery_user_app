...
class ShopController extends ChangeNotifier {
  ...
  String sellerType = "all_seller";
  String sellerTypeTitle = "top_seller";
  String categoryTitle = "";
  void setSellerType(String type, String? category, String? idCategory,
      {bool notify = true}) {
    sellerType = type;
    sellerModel = null;
    if (sellerType == "top") {
    ...
    getTopSellerList(true, 1,
        type: sellerType, category: category, idCategory: idCategory);
    if (notify) {
      notifyListeners();
    }
  }

  SellerModel? sellerModel, searchModel;
  CategoryType? categoryType;

  Future<void> getTopSellerList(bool reload, int offset,
      {required String type, String? category, String? idCategory}) async {
    if (categoryType == CategoryType.restaurant && category == "store" ||
        categoryType == CategoryType.store && category == "restaurant") {
      sellerModel = null;
      // print('-------------ggg--------------');
    }
    ...
    var localData = await database.getCacheResponseById(
        "${AppConstants.sellerList}$type?${category != null ? "category_type=$category" : "category_id=$idCategory"}");
    ...
    ApiResponse apiResponse = await shopServiceInterface!
        .getSellerList(type, category, idCategory, offset, null);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      if (offset == 1) {
        sellerModel = null;
  ...
  Future<void> searchShops({
    ...
    String url =
        "${AppConstants.sellerList}$type?${"category_type=$category"}${query.isNotEmpty ? "&query=$query" : ""}";
    // Check cache
    var localData = await database.getCacheResponseById(url);
    if (localData != null) {
      searchModel = SellerModel.fromJson(jsonDecode(localData.response));
      notifyListeners();
    }

    // API call
    ApiResponse apiResponse = await shopServiceInterface!.getSellerList(
      type,
      category,
      idCategory,
      offset,
    ...
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      if (offset == 1) {
        searchModel = SellerModel.fromJson(apiResponse.response!.data);
      } else {
        searchModel?.sellers?.addAll(
            SellerModel.fromJson(apiResponse.response!.data).sellers ?? []);
        searchModel?.offset =
            SellerModel.fromJson(apiResponse.response!.data).offset!;
        searchModel?.totalSize =
            SellerModel.fromJson(apiResponse.response!.data).totalSize!;
      }
  ...
  Future<void> getAllSellerList(bool reload, int offset,
      {required String type}) async {
    var localData =
        await database.getCacheResponseById("${AppConstants.sellerList}$type");
    ...
    ApiResponse apiResponse = await shopServiceInterface!
        .getSellerList('all', null, null, offset, null);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      if (offset == 1) {
        allSellerModel = null;
        ...
        allSellerModel?.sellers?.addAll(
            SellerModel.fromJson(apiResponse.response!.data).sellers ?? []);
        allSellerModel?.offset =
            (SellerModel.fromJson(apiResponse.response!.data).offset!);
        allSellerModel?.totalSize =
            (SellerModel.fromJson(apiResponse.response!.data).totalSize!);
      }
    }
    notifyListeners();
  ...
  Future<void> getClearanceShopProductList(
    ...
    ApiResponse apiResponse = await shopServiceInterface!
        .getClearanceShopProductList(type, offset, sellerId);
  ...
  Future<ApiResponse> getClearanceSearchProduct(
      ...
      bool fromPaginantion = false}) async {
    ...
    ApiResponse apiResponse = await shopServiceInterface!
        .getClearanceSearchProduct(sellerId, offset.toString(), productId,
  ...
  int? get clearanceSaleProductSelectedIndex =>
      _clearanceSaleProductSelectedIndex;
...
enum CategoryType { store, restaurant }

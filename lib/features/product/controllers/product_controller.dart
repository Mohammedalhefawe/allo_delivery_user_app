...
class ProductController extends ChangeNotifier {
  ...
  List<int> _categoryOffsetList = [];
  int? get categoryPageSize => _categoryPageSize;
  int get categoryOffset => _categoryOffset;
  List<int> get categoryOffsetList => _categoryOffsetList;

  // Discounted pagination
  List<Product> discountedProductList = [];
  int? _discountedPageSize;
  int _discountedOffset = 1;
  List<int> _discountedOffsetList = [];
  int? get discountedPageSize => _discountedPageSize;
  int get discountedOffset => _discountedOffset;

  // Best-selling pagination
  List<Product>? _bestSellingStoreProducts;
  List<Product>? _bestSellingRestaurantProducts;
  int? _bestSellingStorePageSize;
  int? _bestSellingRestaurantPageSize;
  int _bestSellingStoreOffset = 1;
  int _bestSellingRestaurantOffset = 1;
  final List<int> _bestSellingStoreOffsetList = [];
  final List<int> _bestSellingRestaurantOffsetList = [];

  List<Product>? get bestSellingStoreProducts => _bestSellingStoreProducts;
  List<Product>? get bestSellingRestaurantProducts =>
      _bestSellingRestaurantProducts;
  int? get bestSellingStorePageSize => _bestSellingStorePageSize;
  int? get bestSellingRestaurantPageSize => _bestSellingRestaurantPageSize;
  int get bestSellingStoreOffset => _bestSellingStoreOffset;
  int get bestSellingRestaurantOffset => _bestSellingRestaurantOffset;
  ...
  Future<void> getLatestProductList(int offset, {bool reload = false}) async {
    ...
    } else if (_productType == ProductType.categoryProduct) {
      endUrl = AppConstants.categoryProductUri;
    }
  ...
  // Category product with pagination
  Future<void> getProductsByCategoryList(bool reload,
      {required String idCategory, int offset = 1}) async {
    String endUrl =
        '${AppConstants.categoryProductUri}$idCategory?guest_id=1&limit=10&offset=$offset';
    final localData = await database.getCacheResponseById(endUrl);
    ...
    if (localData != null && offset == 1) {
      ...
      if (data['products'] is List) {
        for (var product in (data['products'] as List)) {
          categoryProductList.add(Product.fromJson(product));
        }
        _categoryPageSize = data['total_size'] ?? 0;
        _categoryOffset = offset;
        _filterFirstLoading = false;
        notifyListeners();
      }
    }
    ...
    if (categoryProductList.isEmpty ||
        reload ||
        !_categoryOffsetList.contains(offset)) {
      _categoryOffsetList.add(offset);
      _categoryOffset = offset;
      ...
      ApiResponse apiResponse =
          await productServiceInterface!.getCategoryProductList(endUrl);
      if (apiResponse.response != null &&
          apiResponse.response!.statusCode == 200) {
        if (apiResponse.response!.data['products'] is List) {
          if (offset == 1) {
            categoryProductList.clear();
          }
          for (var product
              in (apiResponse.response!.data['products'] as List)) {
            categoryProductList.add(Product.fromJson(product));
          }
          _categoryPageSize = apiResponse.response!.data['total_size'] ?? 0;
          _categoryOffset = offset;
  ...
  Future<void> getDiscountedProducts({
    ...
    String endUrl =
        '${AppConstants.discountedProductNewUri}?product_type=$productType&discount=$discount&guest_id=1&limit=10&offset=$offset';
    final localData = await database.getCacheResponseById(endUrl);
    ...
    if (localData != null && offset == 1) {
      ...
      if (data['products'] is List) {
        for (var product in (data['products'] as List)) {
          discountedProductList.add(Product.fromJson(product));
        }
        _discountedPageSize = data['total_size'] ?? 0;
        _discountedOffset = offset;
        _filterFirstLoading = false;
        notifyListeners();
      }
    }

    if (discountedProductList.isEmpty ||
        reload ||
        !_discountedOffsetList.contains(offset)) {
      _discountedOffsetList.add(offset);
      _discountedOffset = offset;

      if (reload || offset == 1) {
        discountedProductList.clear();
        _discountedOffsetList = [];
        _discountedOffsetList.add(offset);
      }
      ...
      ApiResponse apiResponse = await productServiceInterface!
          .getCategoryProductList(endUrl); // Reuse existing service method
      if (apiResponse.response != null &&
          apiResponse.response!.statusCode == 200) {
        if (apiResponse.response!.data['products'] is List) {
          if (offset == 1) {
          ...
          _discountedPageSize = apiResponse.response!.data['total_size'] ?? 0;
          _discountedOffset = offset;
  ...
  Future<void> getBestSellingProducts({
    ...
    String endUrl =
        '${AppConstants.bestSellingProductNewUri}?product_type=$productType&guest_id=1&limit=10&offset=$offset';
    final localData = await database
        .getCacheResponseById(endUrl); // Replace with your database call

    // Handle cached data for offset == 1
    if (localData != null && offset == 1) {
      if (productType == 'store') {
        _bestSellingStoreProducts = [];
      } else {
        _bestSellingRestaurantProducts = [];
      }
      final data = jsonDecode(localData.response);
      if (data['products'] is List) {
        for (var product in (data['products'] as List)) {
          if (productType == 'store') {
            _bestSellingStoreProducts!.add(Product.fromJson(product));
          } else {
            _bestSellingRestaurantProducts!.add(Product.fromJson(product));
          }
        }
        if (productType == 'store') {
          _bestSellingStorePageSize = data['total_size'] ?? 0;
          _bestSellingStoreOffset = offset;
        } else {
          _bestSellingRestaurantPageSize = data['total_size'] ?? 0;
          _bestSellingRestaurantOffset = offset;
        }
        _filterFirstLoading = false;
        notifyListeners();
      }
    ...
    final productList = productType == 'store'
        ? _bestSellingStoreProducts
        : _bestSellingRestaurantProducts;
    final offsetList = productType == 'store'
        ? _bestSellingStoreOffsetList
        : _bestSellingRestaurantOffsetList;

    if (productList == null ||
        productList.isEmpty ||
        reload ||
        !offsetList.contains(offset)) {
      ...
      if (reload || offset == 1) {
        if (productType == 'store') {
          _bestSellingStoreProducts = [];
          _bestSellingStoreOffsetList.clear();
          _bestSellingStoreOffsetList.add(offset);
        } else {
          _bestSellingRestaurantProducts = [];
          _bestSellingRestaurantOffsetList.clear();
          _bestSellingRestaurantOffsetList.add(offset);
        }
      }
      ...
      ApiResponse apiResponse = await productServiceInterface!
          .getCategoryProductList(endUrl); // Replace with your API call
      if (apiResponse.response != null &&
          apiResponse.response!.statusCode == 200) {
        if (apiResponse.response!.data['products'] is List) {
          if (offset == 1) {
          ...
          if (productType == 'store') {
            _bestSellingStorePageSize =
                apiResponse.response!.data['total_size'] ?? 0;
            _bestSellingStoreOffset = offset;
          } else {
            _bestSellingRestaurantPageSize =
                apiResponse.response!.data['total_size'] ?? 0;
            _bestSellingRestaurantOffset = offset;
          }
  ...
  List<ProductTypeModel> productTypeList = [
    ...
    ProductTypeModel('category_product', ProductType.categoryProduct),
  ];

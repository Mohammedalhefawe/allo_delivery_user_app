...
class CategoryRepository implements CategoryRepoInterface {
  ...
  @override
  Future<ApiResponse> getRestaurantCategoryList() async {
    try {
      final response =
          await dioClient!.get("${AppConstants.categoryUri}?type=restaurant");
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
  ...
  @override
  Future<ApiResponse> getStoreCategoryList() async {
    try {
      final response =
          await dioClient!.get("${AppConstants.categoryUri}?type=store");
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
